...
flash_attn_stub.flash_attn_interface.flash_attn_func = lambda q, k, v, causal=True: torch.nn.functional.scaled_dot_product_attention(q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2), is_causal=causal).transpose(1, 2)
...
layer_norm_module.layer_norm_fn = lambda hidden_states, weight, bias=None, **kwargs: torch.nn.functional.layer_norm(hidden_states, hidden_states.shape[-1:], weight, bias, kwargs.get("eps", 1e-5))
...
from picotron.model import Qwen3Model as PicotronQwen3Model
...
def _build_tiny_config() -> Qwen3Config:
    return Qwen3Config(
        vocab_size=97,
        hidden_size=64,
...
def test_qwen3_matches_hf():
    ...
    picotron_model = PicotronQwen3Model(cfg).eval()
    ...
    with torch.no_grad():
        ...
        hf_logits = (hf_result.logits if hasattr(hf_result, "logits") else hf_result[0]).to(torch.float32).cpu()
