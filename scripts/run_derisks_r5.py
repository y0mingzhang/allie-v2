#!/usr/bin/env python3
"""R5: Data quality ablations — compare filtering strategies at matched compute."""

import json, os, re, subprocess, tempfile, time, copy
...
# Use full_v1 val for all (fair comparison)
EXPERIMENTS = {
    "baseline": {
        "dataset": {
            "name": "tokens",
            "train_glob": "data/tokens_v2/ablation_baseline/train.npy",
    ...
    "no_short": {
        "dataset": {
            "name": "tokens",
            "train_glob": "data/tokens_v2/ablation_no_short/train.npy",
            "val_glob": "data/tokens_v2/full_v1/val.npy",
            "num_workers": 4,
        }
    },
    "high_elo": {
        "dataset": {
            "name": "tokens",
            "train_glob": "data/tokens_v2/ablation_high_elo/train.npy",
            "val_glob": "data/tokens_v2/full_v1/val.npy",
            "num_workers": 4,
        }
    },
...
def run_experiment(name, config, gpu_offset=0):
    ...
    env.update(
        {
            ...
            "FLASH_ATTEN": "0",
            "CUDA_VISIBLE_DEVICES": ",".join(str(i + gpu_offset) for i in range(4)),
        }
    )
    print(
        f"\n{'=' * 60}\nRunning: {name} (GPUs {gpu_offset}-{gpu_offset + 3})\n{'=' * 60}",
        flush=True,
    )
    start = time.time()
...
def main():
    ...
    for exp_name, overrides in EXPERIMENTS.items():
        ...
        result = run_experiment(name, config, gpu_offset=0)
        results[name] = {"experiment": exp_name, **result}
        with open("results/data_quality_ablation.json", "w") as f:
            json.dump(results, f, indent=2)

    print(f"\n{'=' * 70}\nDATA QUALITY ABLATION RESULTS\n{'=' * 70}", flush=True)
    print(f"{'Name':>20} {'ValLoss':>8} {'MoveAcc':>8} {'Bits/M':>8} {'Time':>6}")
    for name, r in sorted(results.items(), key=lambda x: x[1].get("final_val_loss", 99)):
        bpm = r.get("final_bits_per_move", "?")
        print(
            f"{name:>20} {r.get('final_val_loss', '?'):>8} {r.get('final_move_acc', '?'):>8} {bpm:>8} {r.get('elapsed', 0) / 60:>5.1f}m"
        )
