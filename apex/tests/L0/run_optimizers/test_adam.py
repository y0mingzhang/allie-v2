...
class AdamTest(unittest.TestCase):
    ...
    def testGradScaler(self):
        ...
        for i in range(100):
            ...
            for module in zip(self.model.modules(), self.model_.modules(), strict=False):
                m = module[0]
                m_ = module[1]
                if isinstance(m, nn.Conv2d) or isinstance(m_, nn.Linear):
                    ...
                    torch.testing.assert_close(
                        m.weight.grad, m_.weight.grad, atol=1e-3, rtol=1e-3, equal_nan=True
                    )
    ...
    def testGradScalerCapturableMaster(self):
        ...
        for i in range(100):
            ...
            for module in zip(self.model.modules(), self.model_.modules(), strict=False):
                ...
                if isinstance(m, nn.Conv2d) or isinstance(m_, nn.Linear):
                    torch.testing.assert_close(
                        m.weight, m_.weight.float(), atol=1e-3, rtol=1e-3, equal_nan=True
                    )
                    torch.testing.assert_close(
                        m.weight.grad, m_.weight.grad.float(), atol=1e-3, rtol=1e-3, equal_nan=True
                    )
