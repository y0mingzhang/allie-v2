...
if parse(torch.__version__) >= pytorch_113 or parse(get_nvidia_pytorch_version()) >= ngc_container_2209:
...
def _get_default_world_sizes_model_parallel_world_size(
    pipeline_model_parallel_world_size: int | None = None,
) -> tuple[int, int, int]:
...
class PipelineParallelForwardBackwardTestBase:
    ...
    def _forward_backward_test_impl(
        ...
        pipeline_model_parallel_world_size: int | None,
            ...
            if dtype == get_dtype_for_comparison():
                ...
                if not forward_only:
                    for vm_id, model_module in enumerate(model):
                        ...
                        self.assertEqual(
                            params[0].grad.cpu() / microbatch_size, target_params[0].grad
                        )
                        self.assertEqual(
                            params[1].grad.cpu() / microbatch_size, target_params[1].grad
                        )
...
class NcclPipelineParallelWithCustomSyncContextHandler(NcclDistributedTestBase):
    ...
    @unittest.skipIf(
        torch.cuda.device_count() < 2 or torch.cuda.device_count() % 2 != 0, "Requires >= 2 GPUs"
    )
    ...
    @unittest.skipIf(
        torch.cuda.device_count() < 4 or torch.cuda.device_count() % 2 != 0, "Requires >= 4 GPUs"
    )
