...
"""
    ...
    def emit(self, configuration_name, operations):
        ...
        self.top_level_file.write(
            SubstituteTemplate(
                self.configuration_prototype_template, {"configuration_name": configuration_name}
            )
        )
    ...
    def __exit__(self, exception_type, exception_value, traceback):
        ...
        for configuration_name in self.configurations:
            self.top_level_file.write(
                SubstituteTemplate(
                    self.configuration_template, {"configuration_name": configuration_name}
                )
            )
...
class Manifest:
    ...
    def filter(self, operation):
        ...
        if len(self.operations_enabled) and operation.operation_kind not in self.operations_enabled:
            return False
            ...
            if self.disable_full_archs_compilation:
                ...
                def for_ampere(name):
                    return (
                        ...
                        or "16832" in name
                        or "16864" in name
                        or ("1688" in name and "tf32" in name)
                    )
                ...
                def get_src_archs_str_given_requested_cuda_archs(archs, source_file):
                    ...
                    if intersected_archs == set():
                        raise RuntimeError(
                            f"""
                      Empty archs set for file {source_file} after taking
                      the intersection of {set(self.compute_capabilities)} (global requested archs) and
                      {archs} (per file requested archs)
                      """
                        )
                    else:
                ...
                for source_file in source_files:
                    if is_cpp(source_file):
                        ...
                        archs_str = get_src_archs_str_given_requested_cuda_archs(
                            {80, 87, 90}, source_file
                        )
                        ...
                        raise RuntimeError(
                            f"Per file archs are not set {source_file}, as there is no rule specified for this file pattern"
                        )

                    manifest_file.write(
                        "cutlass_apply_cuda_gencode_flags({} SM_ARCHS {})\n".format(
                            str(source_file.replace("\\", "/")), archs_str
                        )
                    )
