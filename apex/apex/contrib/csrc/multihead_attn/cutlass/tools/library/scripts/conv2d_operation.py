...
"""
    ...
    def emit(self, operation):
        ...
        if operation.group_mode == GroupMode.NoneGroup:
            ...
            values["threadblock_output_shape_n"] = str(
                operation.tile_description.threadblock_output_shape[0]
            )
            values["threadblock_output_shape_p"] = str(
                operation.tile_description.threadblock_output_shape[1]
            )
            values["threadblock_output_shape_q"] = str(
                operation.tile_description.threadblock_output_shape[2]
            )
            ...
            if conv_kind == ConvKind.Fprop or (
                ...
                for output_type in output_types:
                    ...
                    C = TensorDescription(
                        output_type,
                        LayoutType.TensorNHWC,
                        max(1, int(align / DataTypeSize[output_type])),
                    )
