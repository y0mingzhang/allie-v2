...
class Rank2KOperation:
    ...
    def layout_name(self):
        if self.is_complex() or self.is_planar_complex():
            return "%s" % (ShortComplexLayoutNames[(self.A.layout, self.A.complex_transform)])
...
"""
    ...
    def emit(self, operation):
        ...
        rank_k_template = (
            self.rank_k_complex_template if operation.is_complex() else self.rank_k_template
        )
...
"""
    ...
    def emit(self, operation):
        ...
        self.instance_wrappers.append(
            SubstituteTemplate(
                self.instance_template[operation.rank_k_kind],
                {
