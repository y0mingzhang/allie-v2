...
class GemmGroupedSm80(unittest.TestCase):
    def test_SM80_Device_GemmGrouped_f16n_f16t_f32n_tensor_op_f32_128x128x32_64x64x32(self):
        ...
        A = TensorDescription(element=cutlass.float16, layout=cutlass.ColumnMajor, alignment=8)

        B = TensorDescription(element=cutlass.float16, layout=cutlass.ColumnMajor, alignment=8)

        C = TensorDescription(element=cutlass.float32, layout=cutlass.ColumnMajor, alignment=4)
    ...
    def test_SM80_Device_GemmGrouped_f64t_f64t_f64n_tensor_op_f64_64x64x16_32x32x16(self):
        math_inst = MathInstruction(
            ...
            element_accumulator=cutlass.float64,
            opcode_class=cutlass.OpClass.TensorOp,
            math_operation=MathOperation.multiply_add,
        )
        ...
        C = TensorDescription(element=cutlass.float64, layout=cutlass.ColumnMajor, alignment=1)
    ...
    def test_SM80_Device_GemmGrouped_f32t_f32t_f32t_simt_f32_128x64x8_64x32x1(self):
        math_inst = MathInstruction(
            ...
            element_a=cutlass.float32,
            element_b=cutlass.float32,
            element_accumulator=cutlass.float32,
        ...
        A = TensorDescription(element=cutlass.float32, layout=cutlass.RowMajor, alignment=1)

        B = TensorDescription(element=cutlass.float32, layout=cutlass.RowMajor, alignment=1)

        C = TensorDescription(element=cutlass.float32, layout=cutlass.RowMajor, alignment=1)
