...
def EpilogueAlignment(max_alignment, tile, epilogue_steps=8):
    ...
    elements_per_thread = (
        product(tile.threadblock_shape[:-1]) // product(tile.warp_count) // 32 // epilogue_steps
    )
        ...
        for tile_description in tile_descriptions:
            for alignment in alignment_constraints:
                for complex_transform in complex_transforms:
                    ...
                    new_operation = GroupedGemmOperation(
                        GemmKind.Grouped,
                        tile_description.minimum_compute_capability,
                        tile_description,
                        A,
            ...
            if ConvKind.Fprop in conv_kinds:
                ...
                for iterator_algorithm in iterator_algorithms:
                    ...
                    if (
                        ...
                        and A.layout == LayoutType.TensorNHWC
                    ):
                        # SingleGroup kernel
                        ...
                        if iterator_algorithm == IteratorAlgorithm.Analytic:
                            new_operations.append(
                                Conv2dOperation(
                                    ...
                                    epilogue_functor,
                                    swizzling_functor_,
                                    group_mode=GroupMode.MultipleGroup,
                                )
                            )
            ...
            if ConvKind.Dgrad in conv_kinds:
                ...
                if IteratorAlgorithm.Analytic in iterator_algorithms:
                    new_operation = Conv2dOperation(
                        ...
                        StrideSupport.Strided,
                        epilogue_functor,
                        swizzling_functor_strided_dgrad_,
                    )
...
def CreateConv2dFixedChannelsOperator(
    manifest,
    layout,
    tile_descriptions,
    data_type,
    ...
    # iterator algorithm (analytic and optimized)
    iterator_algorithms = [
        IteratorAlgorithm.FixedChannels,
    ]
...
# Convolution for 2D operations specialized for few channels
def CreateConv2dFewChannelsOperator(
    manifest,
    layout,
    tile_descriptions,
    data_type,
...
def CreateDepthwiseConv2dOperator(
    manifest,
    layout,
    tile_descriptions,
    data_type,
...
def GenerateSM75_TensorOp_1688_FewChannels(manifest, cuda_version, math_inst):
    ...
    if math_inst.element_a != math_inst.element_accumulator:
        ...
        CreateConv2dFixedChannelsOperator(
            manifest, conv_layout, tile_descriptions, data_type_mixed, [4, 8]
        )
        CreateConv2dFewChannelsOperator(
            manifest, conv_layout, tile_descriptions, data_type_mixed, [1, 2, 4]
        )
...
def GenerateSM80_TensorOp_16816(manifest, cuda_version):
    ...
    for math_inst in math_instructions:
        ...
        CreateConv2dFixedChannelsOperator(
            manifest, conv_layout, tile_descriptions, data_type, [4, 8]
        )
        ...
        if math_inst.element_a != math_inst.element_accumulator:
            ...
            CreateConv3dOperator(
                manifest, LayoutType.TensorNDHWC, tile_descriptions, data_type_mixed, 8
            )
...
def GenerateSM80_TensorOp_168256(manifest, cuda_version):
    ...
    min_cc = 80
    max_cc = {MathOperation.xor_popc: 1024}
    ...
    for math_inst in math_instructions:
        tile_descriptions = [
            TileDescription(
                [256, 128, 512], 3, [4, 2, 1], math_inst, min_cc, max_cc[math_inst.math_operation]
            ),
            TileDescription(
                [128, 256, 512], 3, [2, 4, 1], math_inst, min_cc, max_cc[math_inst.math_operation]
            ),
            TileDescription(
                [256, 64, 512], 4, [4, 1, 1], math_inst, min_cc, max_cc[math_inst.math_operation]
            ),
            TileDescription(
                [64, 256, 512], 4, [1, 4, 1], math_inst, min_cc, max_cc[math_inst.math_operation]
            ),
            TileDescription(
                [128, 128, 512], 5, [2, 2, 1], math_inst, min_cc, max_cc[math_inst.math_operation]
            ),
            TileDescription(
                [128, 64, 512], 6, [2, 2, 1], math_inst, min_cc, max_cc[math_inst.math_operation]
            ),
            TileDescription(
                [64, 128, 512], 6, [2, 2, 1], math_inst, min_cc, max_cc[math_inst.math_operation]
            ),
            TileDescription(
                [64, 64, 512], 10, [2, 2, 1], math_inst, min_cc, max_cc[math_inst.math_operation]
            ),
            TileDescription(
                [256, 128, 1024], 3, [4, 2, 1], math_inst, min_cc, max_cc[math_inst.math_operation]
            ),
            TileDescription(
                [128, 256, 1024], 3, [2, 4, 1], math_inst, min_cc, max_cc[math_inst.math_operation]
            ),
            TileDescription(
                [256, 64, 1024], 4, [4, 1, 1], math_inst, min_cc, max_cc[math_inst.math_operation]
            ),
            TileDescription(
                [64, 256, 1024], 4, [1, 4, 1], math_inst, min_cc, max_cc[math_inst.math_operation]
            ),
            TileDescription(
                [128, 128, 1024], 4, [2, 2, 1], math_inst, min_cc, max_cc[math_inst.math_operation]
            ),
            TileDescription(
                [128, 64, 1024], 3, [2, 2, 1], math_inst, min_cc, max_cc[math_inst.math_operation]
            ),
            TileDescription(
                [64, 128, 1024], 3, [2, 2, 1], math_inst, min_cc, max_cc[math_inst.math_operation]
            ),
            TileDescription(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="True",
        type=str,
        help="If enabled, kernels whose comupte capability range is not satisfied by the build target are excluded.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--interface-dir", default=None, required=False, help="Interface header to kernels"
    )
    parser.add_argument(
        "--disable-full-archs-compilation",
        action="store_true",
        required=False,
        help="Disable compilation for every archs in --architectures",
    )
