...
def _multi_tensor_copy(
    ...
    for buf_in, buf_out in zip(buffers_in, buffers_out, strict=False):
...
class DistributedFusedAdam(torch.optim.Optimizer):
    ...
    def init_param_buffer(self) -> None:
        ...
        for param, buffer_view in zip(params, param_buffer_views, strict=False):
    ...
    def _local_step(self, bucket_ids: list[int]) -> None:
        ...
        for (group_id, _, _, _), group_buffers in buffers.items():
            ...
            multi_tensor_applier(
                adam_func,
                self._dummy_overflow_buf,
                list(zip(*group_buffers, strict=False)),
                self._grad_scale,
                group["lr"],
                beta1,
                beta2,
    ...
    def _local_step_with_scaled_states(
        ...
        for bucket_id in bucket_ids:
            ...
            for buf, scale in zip(unscaled_buffers, buffer_scales, strict=False):
                buf.mul_(scale)
