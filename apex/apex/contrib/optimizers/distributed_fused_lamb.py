...
class DistributedFusedLAMB(torch.optim.Optimizer):
    ...
    def _lazy_init_stage1(self):
        ...
        self._new_params_mega_shards, self._new_params_mega_blocks, self._new_params_mega_chunks = (
        ...
        if self._full_ar:
            ...
            self._flat_grads_blocks, self._flat_grads_chunks, self._flat_grads_shards = _flat_split(
                self._flat_grads
            )
            # for weight update
    ...
    def _lazy_init_stage2(self):
            ...
            for block_id in range(self._num_blocks):
                for chunk_id in range(self._num_chunks):
                    ...
                    for param_i, (p, grads_info, group_props) in enumerate(
                        zip(self._model_params, self._grads_info, self._group_properties, strict=False)
                    ):
        ...
        p, m, v, u, g, p_copy = list(zip(*self._contrib_tensor_list, strict=False))
        ...
        decay, bias_correction, beta1, beta2, beta3, epsilon = list(
            zip(*self._contrib_group_properties, strict=False)
        )
        ...
        self._packed_flat_to_model_params_fp16 = (
            list(zip(*self._packed_flat_to_model_params_fp16, strict=False))
            if len(self._packed_flat_to_model_params_fp16) > 0
            else None
        )
        self._packed_flat_to_model_params_fp32 = (
            list(zip(*self._packed_flat_to_model_params_fp32, strict=False))
                    ...
                    if self._full_ar:
                        ...
                        l2_grad_norm_sq = (
                            multi_tensor_applier(
                                self.multi_tensor_l2norm, self._overflow_buf, [flat_list], False
                            )[0]
                            ** 2
                        )
                    else:
    ...
    def __compute_contrib_param_norm(self):
        if (
            ...
            gnorm = torch.empty(
                size=[self._contrib_model_param_for_norm_num], dtype=torch.bool, device="cuda"
            )
    ...
    def _pipeline_step(self):
        ...
        if not self._clip_after_ar:
            torch.distributed.all_reduce(
                is_finite, op=torch.distributed.ReduceOp.MIN, group=self._current_process_group
            )
            torch.distributed.all_reduce(
                self._overflow_buf,
        ...
        if len(self._grads_fp16) > 0:
            ...
            if not self._fused_norm:
                multi_tensor_applier(
                    ...
                    self._overflow_buf,
                    list(zip(*self._grads_fp16, strict=False)),
                    scale,
                )
            else:
        ...
        if len(self._grads_fp32) > 0:
            ...
            if not self._fused_norm:
                multi_tensor_applier(
                    amp_C.multi_tensor_scale,
                    self._overflow_buf,
                    list(zip(*self._grads_fp32, strict=False)),
                    scale,
                )
            else:
                self._L2_grad_norm = multi_tensor_applier(
