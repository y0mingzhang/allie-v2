...
class FusedSGD(Optimizer):
    ...
    def step(self, closure=None, grads=None, output_params=None, scale=1.0, grad_norms=None):
        ...
        elif isinstance(output_params, types.GeneratorType) or type(output_params[0]) != list:
            output_params_group = [output_params]
        else:
            output_params_group = output_params

        for group, grads_this_group, output_params_this_group in zip(
            self.param_groups, grads_group, output_params_group, strict=False
        ):
            ...
            fp32_grads = [
                g
                for (p, g) in zip(output_params_this_group, grads_this_group, strict=False)
                if p.dtype == torch.float32
            ]
            fp32_params = [
                p2
                for (p1, p2) in zip(output_params_this_group, group["params"], strict=False)
            ...
            for launch_set, first_run in zip(launch_sets, first_runs, strict=False):
