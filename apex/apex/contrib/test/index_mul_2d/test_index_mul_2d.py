...
class IndexMul2dTest(unittest.TestCase):
    ...
    def test_index_mul_float(self):
        ...
        torch.testing.assert_close(
            self.input1_float.grad, self.input1_float_.grad, atol=1e-3, rtol=1e-3, equal_nan=True
        )
        torch.testing.assert_close(
            self.input2_float.grad, self.input2_float_.grad, atol=1e-3, rtol=1e-3, equal_nan=True
        )
    ...
    def test_index_mul_half(self):
        ...
        torch.testing.assert_close(
            self.input1_half.grad, self.input1_half_.grad, atol=2e-3, rtol=5e-2, equal_nan=True
        )
        torch.testing.assert_close(
            self.input2_half.grad, self.input2_half_.grad, atol=1e-3, rtol=1e-3, equal_nan=True
        )
