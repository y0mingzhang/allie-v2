...
class FusedDenseTest(unittest.TestCase):
    def setUp(self, seed=0):
        ...
        print()
        print(
            f"> input=[{self.batch_size}, {self.in_channels}, {self.in_height}, {self.in_width}]"
        )
        print(
    ...
    def test_conv_bias_relu(self):
        ...
        torch.testing.assert_close(
            self.conv1_.bias.grad, self.conv1.bias.grad, atol=1e-3, rtol=1e-3, equal_nan=True
        )
        torch.testing.assert_close(
            self.conv1_.weight.grad, self.conv1.weight.grad, atol=1e-3, rtol=1e-3, equal_nan=True
        )
    ...
    def test_conv_frozen_scale_bias_relu(self):
        with torch.amp.autocast("cuda", dtype=torch.half):
            out = ConvFrozenScaleBiasReLU(
                self.x, self.conv2.weight, self.scale, self.bias, self.conv_pad, self.conv_stride
            )
            loss = (out.float() ** 2).sum() / out.numel()
        loss.backward()
        with torch.amp.autocast("cuda", dtype=torch.half):
        ...
        torch.testing.assert_close(
            self.conv2_.weight.grad, self.conv2.weight.grad, atol=1e-3, rtol=1e-3, equal_nan=True
        )
        torch.testing.assert_close(self.x_.grad, self.x.grad, atol=1e-3, rtol=1e-3, equal_nan=True)
