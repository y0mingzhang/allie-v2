...
def make_models(
    ...
    *,
    ...
    with torch.no_grad():
        for ref_param, dist_param in zip(dist_model.parameters(), ref_model.parameters(), strict=False):
...
class TestDistributedFusedAdam(NcclDistributedTestBase):
    ...
    def test_matches_pytorch(
        ...
        with torch.cuda.stream(stream):
            for step in range(num_steps):
                ...
                for ref_param, dist_param in zip(ref_model.parameters(), dist_model.parameters(), strict=False):
    ...
    def test_clip_grad_norm(self):
        ...
        xs = [3, 1, 4, 1, 5, 9]
        dys = [1, -1, 1, -1, 1, -1]
        for x, dy in zip(xs, dys, strict=False):
    ...
    def test_checkpoint(
        ...
        if self.rank < min(save_group_size, load_group_size):
            for param_save, param_load in zip(model_save.parameters(), model_load.parameters(), strict=False):
                self.assertRaises(
                    AssertionError,
                    torch.testing.assert_close,
                    param_load,
