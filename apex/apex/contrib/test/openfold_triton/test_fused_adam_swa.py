...
class FusedAdamSWATestCase(unittest.TestCase):
    ...
    def test_fused_update_on_random_data(self):
        ...
        grads = [
            ...
            for d, p in zip(compute_dtypes, state_params, strict=False)
        ]
        ...
        compute_params_gt = [p.clone().to(d) for d, p in zip(compute_dtypes, state_params, strict=False)]
        ...
        compute_params_test = [p.clone().to(d) for d, p in zip(compute_dtypes, state_params, strict=False)]
        ...
        for c, g in zip(compute_params_test, grads, strict=False):
            c.grad = g.clone()
        ...
        for i, (p_gt, p_test, p_origin) in enumerate(
            zip(state_params_gt, state_params_test, state_params, strict=False)
        ):
        ...
        for i, (p_test, p_gt) in enumerate(
            zip(
                ...
                chain(state_params_gt, compute_params_gt, swa_params_gt), strict=False,
            )
        ):
        ...
        for i, (m, m_gt) in enumerate(
            zip(
                ...
                chain(moments_gt, velocities_gt), strict=False,
            )
        ):
