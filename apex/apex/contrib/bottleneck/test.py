...
for stride, o_channel in [(1, 32), (1, 128), (2, 32)]:
    ...
    for i, (w, wgrad) in enumerate(zip(model.w_conv, wgrads, strict=False)):
        print(
    ...
    for p, q in zip(model.parameters(), nhwc_model.parameters(), strict=False):
    ...
    for p, q in zip(model.buffers(), nhwc_model.buffers(), strict=False):
        q.data.copy_(p.data)
    ...
    for i, (w, wgrad) in enumerate(zip(nhwc_model.w_conv, wgrads, strict=False)):
        print(
            f"max error wgrad{i + 1}:",
            (wgrad - w.grad.float()).abs().max().item(),
            "max elem:",
