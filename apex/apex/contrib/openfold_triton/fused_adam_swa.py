...
class FusedAdamSWA(Optimizer):
    def __init__(
        ...
        if not all(
            p.shape == b.shape == s.shape for p, b, s in zip(params, compute_params, swa_params, strict=False)
        ):
            raise ValueError(
                "FusedAdamSWA expects each state in params, bf16_params, abd swa_params to have same shape"
            )
        if not all(p.dtype == s.dtype for p, s in zip(params, swa_params, strict=False)):
