...
def schedule_triton_mha(in_shape, fwd=True):
    ...
    if in_shape == [256, 4, 256, 16] or in_shape == [128, 4, 256, 16] or in_shape == [64, 4, 256, 16] or in_shape == [32, 4, 256, 16]:
        ret = [64, 32, 2, 4] if fwd else [64, 64, 4, 0]
    # [*, 8, 256, 32]
    elif in_shape == [128, 8, 256, 32] or in_shape == [64, 8, 256, 32] or in_shape == [32, 8, 256, 32] or in_shape == [16, 8, 256, 32]:  # DAP1
        ret = [64, 32, 2, 3] if fwd else [128, 64, 8, 1]
    # [*, 8, 128, 32]
    elif in_shape == [256, 8, 128, 32]:  # DAP1
        ret = [64, 64, 4, 3] if fwd else [128, 64, 4, 1]
    elif in_shape == [128, 8, 128, 32] or in_shape == [64, 8, 128, 32] or in_shape == [32, 8, 128, 32]:  # DAP2
