...
class Permutation:
    ...
    def set_permutation_saving_params(
        cls, allow_permutation=False, save_permutation_graph=False, permutation_output_dir="."
    ):
    ...
    @classmethod
    def permute_model(
        cls, model, dump_fx_graph=False, save_dumped_fx_graph="./model_permutation_graph.json"
    ):
        ...
        fx_graph, success_in_build_fx_graph = cls.build_fx_graph(
            model, dump_fx_graph=dump_fx_graph, save_dumped_fx_graph=save_dumped_fx_graph
        )
    ...
    def apply_permutation_in_C_dim(cls, node_name, permutation_sequence, dryrun):
        ...
        if cls.__verbosity > 1 and dryrun:
            print(
                f"[apply_permutation_in_C_dim] Permutation for node: '{node_name}' in C dim"
            )
    ...
    def apply_permutation_in_K_dim(cls, node_name, permutation_sequence, fx_graph, dryrun):
        ...
        if cls.__verbosity > 1:
            print(
                f"[apply_permutation_in_K_dim] Permutation for node: '{node_name}' in K dim"
            )
    ...
    def find_real_children(cls, fx_graph):
        ...
        for node_name in reversed(
            ...
            if cls.__verbosity > 2:
                print(
                    f"[find_real_children] node_name: '{node_name}', children: {node_children}"
                )
        ...
        if cls.__save_permutation_graph:
            cls.save_graph_to_json(
                fx_graph,
                save_dumped_graph_path_with_name=os.path.join(
                    cls.__permutation_output_dir, "./model_graph_find_real_children.json"
                ),
            )  # save the intermediate graph as JSON file for debugging
        return fx_graph
