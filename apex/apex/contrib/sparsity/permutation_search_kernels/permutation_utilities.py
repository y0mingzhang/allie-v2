...
def group_differences(A, B):
    ...
    for g in range(len(Bg)):
        ...
        for i in range(len(group)):
            val = group[i]
            if val not in Ag[g]:
                group_in_a = int(np.where(val == A)[0][0] / 4)
...
def move_permutation_towards(B, A, debug=False):
                ...
                if dst == k2[1]:
                    ...
                    if complement_key in entry_dict:
                        ...
                        return swap_and_correct(
                            B,
                            np.where(entry_dict[k2][0] == B)[0][0],
                            np.where(complement == B)[0][0],
                        )
    ...
    if victim_loner_pair is not None:
        ...
        return swap_and_correct(
            B, np.where(victim_loner_pair[0] == B)[0][0], np.where(victim_loner_pair[1] == B)[0][0]
        )
    ...
    for we in range(len(wrong_entries)):
        ...
        for we2 in range(0, len(wrong_entries)):
            ...
            if cur_entry[1] == pos_swap[2] and cur_entry[2] == pos_swap[1]:
                ...
                return swap_and_correct(
                    B, np.where(cur_entry[0] == B)[0][0], np.where(pos_swap[0] == B)[0][0]
                )
            elif (
    ...
    return swap_and_correct(
        B, np.where(wrong_entries[0][0] == B)[0][0], np.where(candidate_second[0] == B)[0][0]
    )
