...
parser.add_argument(
    "--gpu", default=True, type=str2bool, help="uses a gpu to accelerate the search if possible"
)
parser.add_argument(
    "--check_permutation",
    default=False,
    ...
    for i, strategy in enumerate(args.strategies):
        ...
        # channel swaps
            ...
            if args.unstructured >= 0.0:  # just perform the search on the current matrix
                ...
                min_sparsity, duration = find_minimum_sparsity(
                    result, Channel_Swap, escape_attempts=escape_attempts, verbosity=verbosity
                )
                result = unstructured_prune(result, min_sparsity / 100.0)
