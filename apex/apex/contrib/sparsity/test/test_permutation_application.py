    ...
    def __init__(
        ...
        for c in range(self.num_convs - 1):
            ...
            if self.normalization == "BatchNorm2d":
                self.conv_stack.add_module(
                    f"norm_{c}", torch.nn.BatchNorm2d(self.channels, track_running_stats=False)
                )
                self.expected_K_params += 2
                ...
                self.conv_stack.add_module(
                    f"norm_{c}", torch.nn.SyncBatchNorm(self.channels, track_running_stats=False)
                )
                self.expected_K_params += 2
...
class grouped_convs(torch.nn.Module):
    ...
    def __init__(
        ...
        self.conv_stack = torch.nn.Sequential()
        self.conv_stack.add_module(
            "conv_in", torch.nn.Conv2d(self.channels, self.channels, kernel_size=(3, 3), padding=1)
        )
        ...
        self.conv_stack.add_module(
            "conv_dw",
            torch.nn.Conv2d(
                self.channels, self.channels, kernel_size=(3, 3), padding=1, groups=self.channels
            ),
        )
        ...
        self.conv_stack.add_module(
            "conv_1", torch.nn.Conv2d(self.channels, self.channels, kernel_size=(3, 3), padding=1)
        )
...
class simple_forks_joins(torch.nn.Module):
    ...
    def __init__(
        ...
        self.input_convs.add_module(
            "conv_in0", torch.nn.Conv2d(self.channels, self.channels, kernel_size=(3, 3), padding=1)
        )
        # the next conv can permute along both C and K
        self.expected_C_params += 1
        self.expected_K_params += 2
        self.input_convs.add_module(
            "conv_in1", torch.nn.Conv2d(self.channels, self.channels, kernel_size=(3, 3), padding=1)
        )
...
class different_grouped_convs(torch.nn.Module):
    ...
    def __init__(
        ...
        self.input_conv.add_module(
            "input_conv", torch.nn.Conv2d(self.channels, 128, kernel_size=(3, 3), padding=1)
        )
...
class siblings_poison(torch.nn.Module):
    ...
    def __init__(
        ...
        self.input_conv = torch.nn.Sequential()
        self.input_conv.add_module(
            "input_conv", torch.nn.Conv2d(self.input_shape[1], 128, kernel_size=(3, 3), padding=1)
        )
...
class coparent_poison(torch.nn.Module):
    ...
    def __init__(
        ...
        self.block_b.add_module(
            "conv_b1", torch.nn.Conv2d(128, 128, kernel_size=(3, 3), padding=1, groups=4)
        )
...
class depthwise_child_is_sibling(torch.nn.Module):
    ...
    def __init__(
        ...
        self.block_b = torch.nn.Sequential()
        self.block_b.add_module(
            "conv_b_dw", torch.nn.Conv2d(128, 128, kernel_size=(3, 3), padding=1, groups=128)
        )
        self.block_b.add_module(
...
class module_attribute(torch.nn.Module):
    ...
    def __init__(
        ...
        self.input_conv.add_module(
            "conv_input", torch.nn.Conv2d(self.input_shape[1], 128, kernel_size=(3, 3), padding=1)
        )
...
class one_sparse_sibling(torch.nn.Module):
    ...
    def __init__(
        ...
        self.in_conv.add_module(
            "conv_in", torch.nn.Conv2d(self.input_shape[1], 128, kernel_size=(3, 3), padding=1)
        )
