    ...
    if rampup_batch_size is None:
        ...
        if rank == 0:
            _logger.info(
                ...
                f"size {start_batch_size} to global batch size {global_batch_size} with batch size increments "
...
class ConstantNumMicroBatches(NumMicroBatchesCalculator):
    def __init__(self, global_batch_size, micro_batch_size, data_parallel_size):
        ...
        assert global_batch_size % micro_batch_times_data_parallel == 0, (
            f"global batch size ({global_batch_size}) is not divisible by micro batch size ({micro_batch_size})"
            f" times data parallel size ({data_parallel_size})"
        )
    ...
    def update(self, consumed_samples, consistency_check):
        ...
        if consistency_check:
            assert (
                ...
                "current global "
                f"batch size ({self.current_global_batch_size}) is not divisible by micro-batch-size ({self.micro_batch_size}) times"
