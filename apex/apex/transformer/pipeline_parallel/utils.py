...
def listify_model(model: torch.nn.Module | list[torch.nn.Module]) -> list[torch.nn.Module]:
...
def get_kth_microbatch(batch: list[torch.Tensor] | None, k: int) -> list[torch.Tensor]:
