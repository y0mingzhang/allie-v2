...
def get_tensor_shapes(
    ...
    *,
    tensor_shape: list[int] | torch.Size,
...
def recv_forward(
    tensor_shapes: list[None | list[int]],
    *,
    dtype: torch.dtype | None = None,
    async_comm: bool = False,
    sequence_parallel_enabled: bool = False,
    sync_batch_comm: bool = True,
) -> list[None | torch.Tensor | FutureTensor]:
    input_tensors = []
...
def send_forward(
    output_tensors: torch.Tensor | list[None | torch.Tensor],
    tensor_shapes: list[None | list[int]],
    *,
    dtype: torch.dtype | None = None,
    async_comm: bool = False,
    ...
    for output_tensor, tensor_shape in zip(output_tensors, tensor_shapes, strict=False):
        if tensor_shape is None:
            continue
...
def send_backward(
    input_tensor_grads: torch.Tensor | list[None | torch.Tensor],
    tensor_shapes: list[None | list[int]],
    *,
    dtype: torch.dtype | None = None,
    async_comm: bool = False,
    ...
    for input_tensor_grad, tensor_shape in zip(input_tensor_grads, tensor_shapes, strict=False):
        if tensor_shape is None:
            continue
...
def forward_backward_pipelining_without_interleaving(
    ...
    *,
    forward_only: bool,
    tensor_shape: list[int] | torch.Size | None = None,
    decoder_sequence_length: int | None = None,
    dtype: torch.dtype | None = None,
    ...
    async_comm: bool = False,
    sequence_parallel_enabled: bool = False,
    custom_sync_context_handler: Any | None = None,
    custom_grad_sync_func: Any | None = None,
    sync_batch_comm: bool = True,
    num_micro_batches_with_partial_activation_checkpoints: int | None = None,
    **kwargs,
    ...
    input_tensors: list[None | torch.Tensor] = []
    output_tensors: list[None | torch.Tensor] = []
    losses_reduced: list[None | torch.Tensor] = []
    ###################################################################################################################
    ...
    for i in range(num_warmup_microbatches):
        ...
        cur_microbatch: torch.Tensor | None = get_kth_microbatch(batch, i)
    ...
    if num_microbatches_remaining > 0:
        ...
        input_tensor: list[None | torch.Tensor | FutureTensor] = recv_forward(
            tensor_shapes=recv_tensor_shapes,
            dtype=dtype,
            async_comm=async_comm,
            sync_batch_comm=sync_batch_comm,
    ...
    for i in range(num_microbatches_remaining):
        ...
        if max_outstanding_backprops is not None:
            ...
            checkpoint_activations_micro_batch = None
        cur_microbatch: torch.Tensor | None = get_kth_microbatch(
            batch, i + num_warmup_microbatches
        )
        output_tensor: torch.Tensor | Sequence[torch.Tensor] = forward_step(
            forward_step_func,
            cur_microbatch,
            model,
            input_tensor,
