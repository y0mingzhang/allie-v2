...
def _forward_backward_pipelining_with_interleaving(
    ...
    *,
    ...
    custom_sync_context_handler: Callable | None = None,
    custom_grad_sync_func: Callable | None = None,
    custom_param_sync_func: Callable | None = None,
    ...
    **kwargs,
    ...
    input_tensors: list[list[None | torch.Tensor]] = [[] for _ in range(num_model_chunks)]
    output_tensors: list[list[None | torch.Tensor]] = [[] for _ in range(num_model_chunks)]
    curr_iters: list[int] = [0 for _ in range(num_model_chunks)]
    losses_reduced: list[None | torch.Tensor] = []
    if not forward_only:
        output_tensor_grads: list[list[None | torch.Tensor]] = [
