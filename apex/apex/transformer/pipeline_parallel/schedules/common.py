...
Batch = Union[
    torch.Tensor,
    FutureTensor,
    list[torch.Tensor | FutureTensor],
    tuple[torch.Tensor | FutureTensor, ...],
]
LossFunc = Callable[[torch.Tensor], torch.Tensor]
FwdStepFunc = Callable[[Batch | None, torch.nn.Module], tuple[torch.Tensor, LossFunc]]
...
def build_model(
    ...
    **kwargs: Any,
    ...
    if (
        ...
        msg = (
            f" > number of parameters on (tensor, pipeline) model parallel rank ({parallel_state.get_tensor_model_parallel_rank()}, {parallel_state.get_pipeline_model_parallel_rank()}): {_calc_number_of_params(model)}"
        )
...
def free_output_tensor(
    output_tensors: torch.Tensor | Sequence[torch.Tensor] | None,
...
def custom_backward(output: torch.Tensor, grad_output: torch.Tensor | None) -> None:
    ...
    assert isinstance(grad_output, (torch.Tensor, type(None))), f"grad_outptu == {type(grad_output).__name__}."
...
def forward_step(
    ...
    input_tensor: torch.Tensor | list[torch.Tensor] | None,
    losses_reduced: list[torch.Tensor],
    dtype: torch.dtype,
    disable_autocast: bool = False,
    checkpoint_activations_micro_batch: bool | None = None,
    ...
    # See https://github.com/NVIDIA/Megatron-LM/blob/5ac5571ba0265af4c491ee0af1508ca7589450c6/megatron/model/transformer.py#L679
...
def backward_step(
    ...
    *,
    ...
    deallocate_pipeline_outputs: bool = False,
) -> None | torch.Tensor | Sequence[torch.Tensor]:
