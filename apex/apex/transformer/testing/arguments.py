...
def parse_args(extra_args_provider=None, defaults={}, override_args={}, ignore_unknown_args=False):
    ...
    parser.add_argument(
        "--cpu-offload", action="store_true", default=False, help="Turns on CPU offloading"
    )
    ...
    assert args.world_size % model_parallel_size == 0, (
        "world size is not"
        f" divisible by tensor parallel size ({args.world_size}) times pipeline parallel "
        f"size ({args.tensor_model_parallel_size})"
    )
    args.data_parallel_size = args.world_size // model_parallel_size
    if args.rank == 0:
        print(
            f"using world size: {args.world_size}, data-parallel-size: {args.data_parallel_size}, "
    ...
    for key in defaults:
        ...
        if getattr(args, key) is not None:
            if args.rank == 0:
                print(
                    f"WARNING: overriding default arguments for {key}:{defaults[key]} \
                       with {key}:{getattr(args, key)}",
                    flush=True,
                )
        else:
