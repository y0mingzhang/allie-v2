...
def initialize_model_parallel(
    ...
    virtual_pipeline_model_parallel_size_: int | None = None,
    pipeline_model_parallel_split_rank_: int | None = None,
    use_fp8_: bool = False,
    ...
    *,
    ...
    if default_backend == "ucc":
        warnings.warn(
            "The UCC's functionality as `default_backend` is not well verified", ExperimentalWarning
        )
    ...
    if torch.distributed.get_rank() == 0:
        _logger.info(
            f"> initializing tensor model parallel with size {tensor_model_parallel_size}"
        )
        _logger.info(
            f"> initializing pipeline model parallel with size {pipeline_model_parallel_size}"
        )
        _logger.info(f"> initializing data parallel with size {data_parallel_size}")
